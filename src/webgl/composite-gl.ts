...
export function compositeLayersGL(
  ...
  const { gl } = state;
  const w = canvas.width;   // logical canvas size (visible to user)
  const h = canvas.height;

  // ── Step 1: ensure FBOs match canvas size ──────────────────
  // ensureGLStateSize recomputes renderScale and reallocates destTexture
  // if either the logical size OR the derived render size changed.
  if (!ensureGLStateSize(state, w, h)) {
    if (typeof console !== 'undefined' && console.warn) {
      console.warn('[GenTonik WebGL] ensureGLStateSize failed', {
        w, h, canvasW: state.canvasW, canvasH: state.canvasH,
        renderW: state.renderW, renderH: state.renderH, lost: state.lost,
      });
    }
    return false;
  }
  // v2.15.2: ping-pong FBOs live at renderW × renderH (physical), not w × h.
  // The final blitFramebuffer upscales from renderW × renderH to w × h.
  const pp = ensureDestPingPong(state, state.renderW, state.renderH);
  if (!pp) {
    if (typeof console !== 'undefined' && console.warn) {
      console.warn('[GenTonik WebGL] ensureDestPingPong failed', {
  ...
  try {
    ...
    // Viewport/scissor use renderW × renderH (the FBO's physical size).
    ...
    // v2.15.2: source rect is renderW × renderH (FBO physical size),
    // dest rect is w × h (logical canvas size). blitFramebuffer with
    // NEAREST filtering handles the upscale; the visible canvas's own
    // scaling (CSS) handles any further browser-level zoom.
    ...
    gl.scissor(0, 0, w, h);
    gl.blitFramebuffer(
      0, 0, state.renderW, state.renderH,  // src: physical FBO
      0, 0, w, h,                            // dst: logical canvas
      gl.COLOR_BUFFER_BIT,
      gl.NEAREST,
    );
    return true;
...
function compositeSingleLayerGL(
  ...
  if (useCanvasClip && canvasSpacePolygon) {
    // v2.15.2: pass renderW/renderH (physical) — the clip texture is
    // rasterized at FBO resolution, NOT logical canvas resolution.
    canvasClipTex = getCanvasClipTexture(state, layer.id, canvasSpacePolygon, state.renderW, state.renderH);
    if (!canvasClipTex) {
  ...
  //
  // v2.15.2: blit dimensions are renderW × renderH (FBO physical size),
  // NOT canvasW × canvasH. The FBOs are allocated at physical size to
  // support oversized documents via renderScale downscaling.
  ...
  // v2.15.2: Physical FBO render size. Used by COMPOSITE_FRAG to convert
  // gl_FragCoord → mask UV for canvas-clip sampling. gl_FragCoord is in
  // the FBO's pixel space (0..renderW, 0..renderH), NOT logical canvas
  // space — using u_canvasSize here would produce wrong UVs when
  // renderScale < 1.0 (oversized documents).
  ...
  // v2.15.2: COMPOSITE_FRAG now uses u_renderSize (NOT u_canvasSize) for
  // the canvas-clip UV — gl_FragCoord is in render-pixel space.
...
interface ClipTextureCacheEntry {
  ...
  /**
   * v2.15.2: Hash of polygon points + renderScale — detects mask AND
   * render-scale changes. We use renderScale (not canvasW/canvasH)
   * because the clip texture lives in render-pixel space: two docs
   * with the same renderScale produce identical clip textures even
   * if their logical canvas sizes differ slightly.
   */
  hash: string;
  /** Render size when this texture was created — detects resize. */
  canvasW: number;
  canvasH: number;
}
...
/**
 * Compute a hash of the polygon + renderScale for cache lookup.
 * Uses a simple string concatenation — fast enough for typical polygon
 * sizes (16-64 points from ellipse/lasso selections).
 *
 * v2.15.2: parameter is renderScale (a single number), not the (canvasW,
 * canvasH) pair. Two docs with the same renderScale produce identical
 * clip textures (the rasterization is in render-pixel space), so we
...
function getCanvasClipTexture(
  ...
  // v2.15.2: hash by renderScale only (not canvasW/canvasH). The clip
  // texture lives in render-pixel space, so two docs with the same
  // renderScale but different logical sizes produce identical textures.
  ...
  if (existing) {
    // Cache hit — check if polygon or render size changed.
    // v2.15.2: hash already includes renderScale, so we only need to
    // check renderW/renderH (physical) for size-change detection.
    if (existing.hash === hash && existing.canvasW === canvasW && existing.canvasH === canvasH) {
      return existing.texture;
    }
    // Polygon or render size changed — delete old texture, create new.
 ...
 * and upload it as a GL texture.
 *
 * v2.15.2: The polygon is in CANVAS-pixel space (0..canvasW, 0..canvasH),
 * but the offscreen rasterization canvas and resulting GL texture are at
 * RENDER-pixel size (0..renderW, 0..renderH). We scale polygon points by
 * state.renderScale before drawing to match the FBO's coordinate system.
 *
 * The composite-quad shader samples this texture with Y flipped (WebGL Y
 * is bottom-up) to clip the layer to the polygon shape. UV math uses
 * u_renderSize (NOT u_canvasSize) so gl_FragCoord maps correctly.
 *
 * LIMITATION: For oversized documents (renderScale < 1.0), clip mask
 * edges are softer than at full resolution. 1 render-pixel = 1/renderScale
 * canvas-pixels of blur. Acceptable for typical selection shapes;
 * bake/export path uses Canvas2D at full res to avoid this.
 *
...
function rasterizeCanvasClipToTexture(
  ...
  const { gl } = state;
  // v2.15.2: canvasW/canvasH here are RENDER size (passed by caller).
  // For non-oversized docs renderScale=1.0, so canvasW === state.canvasW.
  if (polygon.length < 3 || canvasW <= 0 || canvasH <= 0) return null;
  ...
  try {
    ...
    // v2.15.2: Scale polygon from logical canvas space to physical render
    // space. When renderScale=1.0 this is a no-op; for oversized docs it
    // shrinks the polygon to fit the smaller render texture.
...
/**
 * v2.15.3 (KIMI review): Module-level constant for the minimum acceptable
 * preview quality before falling back to Canvas2D.
 *
 * When a document's max dimension exceeds gpuMaxTextureSize × (1/this),
 * we skip WebGL entirely and use Canvas2D directly. Below this threshold,
 * renderScale downscaling produces acceptable preview quality.
 *
 * - 0.25 = 25% preview quality (e.g. 32000px doc on 16384-max GPU → 16384px render)
 * - 0.10 = 10% preview quality (more aggressive WebGL usage, blurrier preview)
 * - 0.50 = 50% preview quality (more conservative, Canvas2D fallback sooner)
 *
 * Tuned to 0.25 as a heuristic — matches the threshold KIMI's review accepted.
 * Adjust here if preview quality vs Canvas2D tradeoff needs retuning.
 */
const MAX_DOWNSCALE_FACTOR = 0.25;
...
export function compositeLayersWithFallback(
  ...
  // v2.15.2: Determine max supported texture size. We can't read
  // state.caps.maxTextureSize yet (state isn't created), so we probe
  // the GPU directly. This is a cheap GL call (~microseconds) and the
  // result is cached by the driver. We use this ONLY for the early
  // fallback decision — once a GLState is created, all subsequent
  // decisions use state.caps.maxTextureSize.
  //
  // NOTE: compositeLayersGL has its own oversized-doc handling via
  // renderScale (downscale FBO + blit upscale). This early fallback
  // is a SAFETY NET for cases where even the downscaled render size
  // would be too large (e.g. >32K doc on a GPU with 4K maxTextureSize),
  // OR when WebGL2 is unavailable entirely.
  let gpuMaxTextureSize = 16384; // safe default; will be probed if WebGL works
  try {
    const probeCanvas = document.createElement('canvas');
    const probeGl = probeCanvas.getContext('webgl2');
    if (probeGl) {
      gpuMaxTextureSize = probeGl.getParameter(probeGl.MAX_TEXTURE_SIZE) as number;
      // Lose the probe context immediately to free resources.
      const loseExt = probeGl.getExtension('WEBGL_lose_context');
      loseExt?.loseContext();
    }
  } catch {
    // WebGL2 probe failed — keep the safe default of 16384.
  }

  // v2.15.2: Early fallback for documents where EVEN the downscaled
  // render would exceed GPU limits. compositeLayersGL handles downscaling
  // gracefully, but if the doc is so large that even floor(maxTex * 1.0)
  // wouldn't help (e.g. >32K on a 4K-max GPU), we skip WebGL entirely.
  // Threshold: if max dimension > 4× gpuMaxTextureSize, the downscale
  // factor would be < MAX_DOWNSCALE_FACTOR (0.25), making preview quality
  // unacceptable.
  //
  // v2.15.3 (KIMI review): ALSO check that no individual layer exceeds
  // gpuMaxTextureSize. Without this check, WebGL would init successfully
  // (destFBO fits), but acquireLayerFBO would return null for the oversized
  // layer (see gl-resources.ts:92), compositeSingleLayerGL would return false,
  // and we'd fall back to Canvas2D anyway — wasting the WebGL init cost.
  // By checking layer sizes upfront, we skip WebGL entirely when any layer
  // is too large, going straight to Canvas2D.
