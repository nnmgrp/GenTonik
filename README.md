...
2. **WebGL-композитинг, масштабирование (Downscaling) и стабильность:** Высокопроизводительный рендеринг слоев на видеокарте, обработка потери контекста WebGL (restoration), нативный скос (perspective/affine) в шейдерах. Внедрено автоматическое масштабирование рендеринга (render scaling) для документов, приближающихся к аппаратному пределу GPU (`maxTextureSize`), во избежание крашей контекста.
...
5. **Инструмент выравнивания слоя (Straighten):** Возможность выравнивания растрового слоя по горизонтали или вертикали (оси X/Y) на основе угла наклона, полученного инструментом Measure.
6. **Инструмент Zoom (🔍):** Масштабирование кликом (Alt+клик для отдаления) или выделением рамки (marquee).
7. **Инструмент Bucket Fill (Заливка 🪣):** 5 режимов заливки цветом или скринтоном всего холста/выделенной области.
8. **Bake Transform / Растеризация:** Конвертация скринтонов с примененными трансформациями в растровые изображения. Внедрено корректное смешивание полупрозрачных цветов (Premultiplied Alpha) при обратной проекции с билинейной фильтрацией, убирающее темную/цветную кайму на краях.
9. **Система масок:** Поддержка растровых (layer-local) и векторных (canvas-space) масок из выделения (с выбором режима проецирования).
10. **Прозрачные слои (Transparent):** Добавлен тип пустых прозрачных слоев (`transparent`) для наложения масок и трансформирования.
11. **История (Undo/Redo):** Полноценная отмена/возврат операций, включая восстановление выделений («бегущих муравьев») и физические кнопки на панели.
12. **Совместимость ORA:** Экспорт и импорт проектов в открытый формат OpenRaster (с сохранением структуры слоев, масок и метаданных).
13. **Адаптивный Canvas2D-fallback:** Интеллектуальный обход WebGL на Canvas2D с предварительным расчетом размеров слоев (например, для огромных сканов `12000×17000` и выше) во избежание сбоев GPU.
