...
> Обновлено: 2026-07-04 (WebGL context loss, persistent canvas, culling, FT singularity & oversized doc analysis)
> Статус: Фаза стабилизации — WebGL context fixes внедрены (v2.18), задокументированы баги Free Transform и сверхбольших документов.
...
- **JPG / WebP / PNG**: Полностью поддерживаются браузером из коробки. Добавление слоев работает через `<input accept="image/*">`, все эти форматы автоматически распознаются и декодируются.
- **AVIF (.avif)**: Поддержка кодирования/декодирования временно удалена во избежание раздувания размера сборки из-за WebAssembly-модуля `@jsquash/avif` (размер bundle возрастал с 750 КБ до 11.7 МБ). Планируется перенос в Tauri в качестве системного плагина/библиотеки.
...
## 11. Внедрённые исправления WebGL (v2.18) ✅
1. **Единый холст (Canvas Reuse):** Убран `key={activeDocId}` с компонента `<CanvasView>`. Холст теперь постоянный (Persistent Canvas), что предотвращает утечку контекстов WebGL (лимит браузеров 8-16) при быстром переключении или создании вкладок.
2. **Конечный автомат рендерера (`rendererState`):** Отслеживает состояния `ready`, `lost` (с ожиданием восстановления 5 секунд) и `disabled`.
3. **Ремаунт холста по ключу `canvasKey`:** Внедрен в `<canvas key={canvasKey}>`. Пересоздает DOM-ноду только при фатальном сбое WebGL, что дает возможность успешно инициализировать `canvas.getContext('2d')` для Canvas2D-фоллбека (HTML5-спецификация запрещает получать 2D контекст на холсте, где ранее опрашивался WebGL).
4. **Коллбэк `onWebGLFallback`:** Интегрирован в `compositeLayersWithFallback` для инкрементирования `canvasKey` при сбое создания `GLState`.
5. **Viewport Culling (Оптимизация):** `compositeCtx` теперь рассчитывает вьюпорт. Слои вне видимого экрана полностью пропускаются при рендеринге FBO и масок, давая прирост FPS на многослойных сценах.

---

## 12. Найденные новые баги для исправления (План на будущее) 🔧
1. **Баг сингулярности Free Transform (Krita-style Clamping):**
   - *Проблема:* Тянущийся угол в режиме `perspective` вызывает вертикальное «спагетти» скринтона из-за деления на околонулевой знаменатель $W \to 0$ гомографии, а `normalizeCorners` в `src/homography.ts` ошибочно меняет индексы углов местами, приводя к гистерезису и сдвигу геометрии.
   - *План исправления:* 
     - Зафиксировать порядок индексов вершин `[TL, TR, BR, BL]` во время драга (убрать `normalizeCorners` из `computeDragResult` в `src/transform-overlay-canvas.tsx`).
     - Внедрить в `src/homography.ts` геометрическое ограничение: не давать перетаскиваемой вершине пересекать противоположную диагональ или смежные грани ближе чем на 8–10 пикселей (скользить вдоль барьера).
     - Добавить в шейдер зажим знаменателя $W$ снизу до `0.0001` (с сохранением знака).
2. **Баг рендеринга больших форматов (FBO Scaling):**
   - *Проблема:* Документ формата 12 000 × 17 000 px сжимает холст через `renderScale` (например, до 11 565 × 16 384 px), но в `compositeSingleLayerGL` (файл `src/webgl/composite-gl.ts`) FBO слоев (включая Solid Background) все равно запрашиваются в полном разрешении (12 000 × 17 000). Видеокарта отвергает текстуру больше `maxTextureSize`, и приложение уходит в медленный Canvas2D OOM-фоллбек.
   - *План исправления:* Масштабировать размеры слоев `renderW` и `renderH` на коэффициент `state.renderScale` в `compositeSingleLayerGL` перед вызовом `acquireLayerFBO` и отрисовкой контента/маски.
